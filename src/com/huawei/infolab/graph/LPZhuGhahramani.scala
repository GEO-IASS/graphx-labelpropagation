...
class LPZhuGhahramani(g:Graph[LPVertex, Double], self_weight:Double=1.0, undirected:Boolean=false, labels:Array[String]) extends java.io.Serializable{
  /**
    * Run label propagation for a fixed number of iterations returning a
    * graph with vertex attributes containing the estimated label probabilities.
    * @param maxIter The numberof iteration the label progation to run
    * @return
    */
  def run(maxIter:Int):Graph[LPVertex, Double] = {
    ...
    def updateVertexEstimatedLabes(vert:LPVertex, msgSum:Map[String, Double]): LPVertex = {
      if(vert.isSeedNode){
        ...
        vert.estimatedLabels = estimatedLabels ++ msgSum.map({case(k,v) => k -> (v + estimatedLabels.getOrElse(k, 0.0))})
        vert.estimatedLabels = initializer.normalize(vert.estimatedLabels)
      }
      vert
    }
